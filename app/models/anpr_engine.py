...
class ANPREngine:
    def __init__(self):
        ...
        vehicle_path = os.getenv("VEHICLE_MODEL_PATH", "weights/vehicle_model.pt")
        plate_path = os.getenv("PLATE_MODEL_PATH", "weights/plate_model.pt")
        self.vehicle_model = YOLO(vehicle_path) if os.path.exists(vehicle_path) else None
        self.plate_model = load_yolov5(plate_path, device=self.device) if os.path.exists(plate_path) else None
